# SCH1150.SH --> JB_INSTALLEDBASE_BOOKINGS.py
...
#
# Description  :
#               1. This script will load the data into 'INSTALLEDBASE_BOOKINGS' table based on stream lookups.
#
#
# Initial Creation:
#
...
app_name = "JB_INSTALLEDBASE_BOOKINGS"
...
def query_data(db_schema):
        ...
        query = """INSERT INTO """+db_schema+""".INSTALLEDBASE_BOOKINGS
            Select * FROM """+ db_schema +""".STG_INSTALLEDBASE_BOOKINGS a
            Where
            ---a.SALES_ORDER_NUMBER = 600113254 AND
            INSTALL_CUST_NAGP_MATCH_PCT < 70 AND
            SERIAL_OWNER_NAGP_MATCH_PCT < 70 AND
            SERIAL_OWNER_NAME_MATCH_PCT < 70 AND
            UPPER(ACCOUNT_NAGP) NOT LIKE '%' || UPPER(INSTALLED_AT_CUSTOMER_NAGP) ||'%' AND
            UPPER(ACCOUNT_NAGP) NOT LIKE '%' || UPPER(SERIAL_NUMBER_OWNER_NAGP) ||'%' AND
             UPPER(ACCOUNT_NAGP) NOT LIKE '%' || UPPER(SERIAL_NUMBER_OWNER_NAME) ||'%'
            ORDER BY SALES_ORDER_NUMBER,SERIAL_NUMBER,SALES_REP_NUMBER"""
